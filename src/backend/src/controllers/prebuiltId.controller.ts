...
class IdController {
  public index = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const url : string = `http://${envConfig.ID_HOSTNAME}:${envConfig.ID_PORT}/formrecognizer/v2.1/prebuilt/idDocument/analyze?includeTextDetails=false&pages=1`
      logger.info(url)
      const config = {
        headers : {
          accept : "*/*",
  ...
  public getResponse = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const url : string = `http://${envConfig.ID_HOSTNAME}:${envConfig.ID_PORT}/formrecognizer/v2.1/prebuilt/idDocument/analyzeResults/${req.params.id}?includeTextDetails=false&pages=1`
